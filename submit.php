...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    $txt = $conn->prepare("INSERT INTO account (`Full Name`, Username, Email, `Contact Number`, Password, Purpose) VALUES (?, ?, ?, ?, ?, ?)");
   
    if ($txt === false) {
        die("Prepare failed: " . $conn->error);
    }
    $txt->bind_param("ssssss", $name, $uname, $email, $contact, $hashedPassword, $purpose);
